...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 如果没有缓存，生成缓存 */
        ...
        if ($count > 0)
        {
            ...
            /* 设置分页链接 */
            $pager = get_pager('busers.php', array('act' => 'list'), $count, $page, $size);
...
elseif ($_REQUEST['act'] == 'view')
{
     ...
     while ($row = $db->fetchRow($res))
    {
        ...
        $smarty->assign('promotion_info', get_promotion_info());
        $smarty->assign('feed_url',         ($_CFG['rewrite'] == 1) ? "feed-typegroup_buy.xml" : 'feed.php?type=group_buy'); // RSS URL
    $smarty->assign('goods', $buserinfo[0]); 
    $smarty->display('busersinfo.dwt', $cache_id);
}
...
function group_buy_list($size, $page)
{
  ...
  //       }
        /* 处理链接 */
        $group_buy['url'] = "busers.php?act=view&id=".$group_buy['user_id'];
        $len=strlen($group_buy['Field_address']);
        if($len>48)$group_buy['Field_address'] = substr($group_buy['Field_address'],0,48).'...';
        /* 加入数组 */
        $gb_list[] = $group_buy;
    }
